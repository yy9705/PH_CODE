...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueTU != cur->valueTU)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueTU Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valuePH = exchangeBytes(gizY2XFloat(PH_RATIO,  PH_ADDITION, dataPoints->valuePH));     
    devStatusPtr->valueTU = exchangeBytes(gizY2X(TU_RATIO,  TU_ADDITION, dataPoints->valueTU)); 
    devStatusPtr->valueTEMP = exchangeBytes(gizY2XFloat(TEMP_RATIO,  TEMP_ADDITION, dataPoints->valueTEMP));     
    devStatusPtr->valueTDS = exchangeBytes(gizY2X(TDS_RATIO,  TDS_ADDITION, dataPoints->valueTDS)); 
